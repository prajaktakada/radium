...
const updateReviews= async function (req, res) {
    try {
        ...
        if (!isValidObjectId(bookId)) {
            res.status(400).send({ status: false, message: `${blogId} is not a valid book id` });
            return;
        }
        ...
        const findBooks = await BookModel.findOne({_id: bookId,isdeleted: false,deletedAt: null});
